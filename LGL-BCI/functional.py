    ...
    for i in range(batch_size):
        for j in range(channels):
            if(eig_mode=='eig'):
                ...
                s, U[i, j] = th.linalg.eigh(P[i,j][None,:])
                S[i, j]    = s[:,0]
            elif(eig_mode=='svd'):
...
def modeig_backward(dx,U,S,S_fn,op,param=None):
    ...
    dp            = U.to(torch.float32).matmul(dp.to(torch.float32)).matmul(U.transpose(2,3).to(torch.float32))
    return dp
...
def CongrG(P,G,mode):
    ...
    PP= GG.to(torch.float32).matmul(P.to(torch.float32)).matmul(GG.to(torch.float32))
    return PP
