...
n_trials_per_class = CONSTANT['n_trials_2_class']
...
def subject_setting(k_folds, pick_smp_freq, bands, save_path, num_class=2, scenario='CV', sel_chs=None):
    sel_chs = CONSTANT['sel_chs'] if sel_chs == None else sel_chs
    n_folds = k_folds
    save_path_1 = save_path + '/OpenBMI/data_CV_S1'
    save_path_2 = save_path + '/OpenBMI/data_CV_S2'
    save_path_3 = save_path + '/OpenBMI/data_Holdout'
    n_chs = len(sel_chs)
    ...
    for s in range(n_subjs):
        X_train, y_train, X_test, y_test = __load_OpenBMI(raw_path, s + 1, id_chosen_chs)
        "T.mat"
    ...
    for person, (X_tr, y_tr, X_te, y_te) in enumerate(zip(X_train_all, y_train_all, X_test_all, y_test_all)):
        ...
        if scenario == 'CV':
            ...
            #
            #     fbank = FilterBank(fs=1000, bands=bands, pass_width=4)
            #     _ = fbank.get_filter_coeff()
            #     '''The shape of x_fb is No. of (trials, frequency bands, channels, timestamps)'''
            #     X_tr_fil = _tensor_stack(
            #         fbank.filter_data(X_tr_cv, window_details={'tmin': 0, 'tmax': 4}).transpose(1, 0, 2, 3))
            #     X_te_fil = _tensor_stack(
            #         fbank.filter_data(X_te_cv, window_details={'tmin': 0, 'tmax': 4}).transpose(1, 0, 2, 3))
            #
            ...
            for fold, (train_index, test_index) in enumerate(skf.split(X_te, y_te)):
                ...
                '''The shape of x_fb is No. of (trials, frequency bands, channels, timestamps)'''
                X_tr_fil = _tensor_stack(
                    fbank.filter_data(X_tr_cv, window_details={'tmin': 00, 'tmax': 4}).transpose(1, 0, 2, 3))
                X_te_fil = _tensor_stack(
                    fbank.filter_data(X_te_cv, window_details={'tmin': 0, 'tmax': 4}).transpose(1, 0, 2, 3))
            ...
            '''The shape of x_fb is No. of (trials, frequency bands, channels, timestamps)'''
            X_tr_fil = _tensor_stack(
                fbank.filter_data(X_tr, window_details={'tmin': 0, 'tmax': 4}).transpose(1, 0, 2, 3))
            X_te_fil = _tensor_stack(
                fbank.filter_data(X_te, window_details={'tmin': 0, 'tmax': 4}).transpose(1, 0, 2, 3))
...
def __load_OpenBMI(PATH, subject, id_chosen_chs):
    X_s1, y_s1, X_s2, y_s2 = raw.get_data(
        PATH=PATH, subject=subject, id_chosen_chs=id_chosen_chs)
...
def _tensor_stack(x_fb):
    time_seg = [[0, 1500], [500, 2000], [1000, 2500]]
    temporal_seg = []
    for [a, b] in time_seg:
