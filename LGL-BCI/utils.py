...
class DataLoader:
    def __init__(self, dataset, data_source='T', subject=None, data_format=None, dataset_path='/datasets', **kwargs):
        ...
        if data_source == 'T':
            self.path = self.dataset_path + '/' + self.dataset + '/data_CV_T'
        elif data_source == 'E':
            self.path = self.dataset_path + '/' + self.dataset + '/data_CV_E'
        else:
            self.path = self.dataset_path + '/' + self.dataset + '/data_Holdout'
    ...
    def load_train_set_holdout(self, ):
        # load
        X, y = np.array([]), np.array([])
        try:
            self.file_x = self.path + '/X_train_Holdout.npy'
            self.file_y = self.path + '/y_train_Holdout.npy'
            X = self._change_data_format(np.load(self.file_x))
            y = np.load(self.file_y)
        except:
            raise Exception(
    ...
    def load_test_set_holdout(self, ):
        # load
        X, y = np.array([]), np.array([])
        try:
            self.file_x = self.path + '/X_test_Holdout.npy'
            self.file_y = self.path + '/y_test_Holdout.npy'
            X = self._change_data_format(np.load(self.file_x))
            y = np.load(self.file_y)
        except:
            raise Exception(
...
class dataloader_in_main(torch.utils.data.Dataset):
...
class DataLoader_OpenBMI:
    def __init__(self, dataset, data_source='S1', subject=None, data_format=None, dataset_path='/datasets', **kwargs):
        ...
        for k in kwargs.keys():
            self.__setattr__(k, kwargs[k])
        if data_source == 'S1':
            self.path = self.dataset_path + '/' + self.dataset + '/data_CV_S1'
        elif data_source == 'S2':
            self.path = self.dataset_path + '/' + self.dataset + '/data_CV_S2'
        else:
            self.path = self.dataset_path + '/' + self.dataset + '/data_Holdout'
