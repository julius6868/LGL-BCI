...
import operator
import functools
from Net.geoopt import utils, tensor
...
class ProductManifold(Manifold):
    ...
    def component_inner(self, x: torch.Tensor, u: torch.Tensor, v=None) -> torch.Tensor:
        ...
        for i, manifold in enumerate(self.manifolds):
            point = self.take_submanifold_value(x, i)
            u_vec = self.take_submanifold_value(u, i)
            target_shape = utils.broadcast_shapes(point.shape, u_vec.shape)
            if v is not None:
                v_vec = self.take_submanifold_value(v, i)
            else:
                v_vec = None
    ...
    def random_combined(
        ...
        return tensor.ManifoldTensor(tensor, manifold=self)
    ...
    def __init__(
        ...
        *manifolds_with_shape: Tuple[Stereographic, Union[Tuple[int, ...], int]],
        ...
        for man in self.manifolds:
            if not utils.ismanifold(man, Stereographic):
